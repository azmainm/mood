...
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=["https://mood-gray-gamma.vercel.app/","https://d365-103-220-204-28.ngrok-free.app","https://moodtracker-68joofshg-azmain-morsheds-projects.vercel.app"],  # Or https://mood-gray-gamma.vercel.app/","https://d365-103-220-204-28.ngrok-free.app","https://moodtracker-68joofshg-azmain-morsheds-projects.vercel.app" to allow all origins
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://mood-gray-gamma.vercel.app/","https://95a5-103-220-204-29.ngrok-free.app","https://moodtracker-cyk57s4vp-azmain-morsheds-projects.vercel.app"],  # Or https://mood-gray-gamma.vercel.app/","https://d365-103-220-204-28.ngrok-free.app","https://moodtracker-68joofshg-azmain-morsheds-projects.vercel.app" to allow all origins
    allow_credentials=True,
    allow_methods=["*"],  # Allow all methods (POST, GET, etc.)
    allow_headers=["*"],  # Allow all headers
)
...
def login(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    # Return username and userID to the frontend
    return {
        "message": "Login successful",
        "username": db_user.username,
        "userID": db_user.id
...
def submit_mood(
    ...
    current_user = crud.get_user_by_username(db, username)
    if not current_user:
...
def get_user_stats(request: Request, db: Session = Depends(get_db)):
    ...
    if not moods:
        raise HTTPException(status_code=404, detail="No mood data available.")
...
@app.get("/moods/last7days")
def get_last_7_days_moods(request: Request, db: Session = Depends(get_db)):
    username = request.headers.get("username")
    if not username:
        raise HTTPException(status_code=401, detail="User not authenticated")
